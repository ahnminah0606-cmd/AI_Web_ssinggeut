...
export async function ai(u:any,input:any[],context:string,taskInstructions="",schema?:any){
 ...
 const res=await write('INSERT INTO limits(user_id,day,count) VALUES(?,?,1) ON CONFLICT(user_id,day) DO UPDATE SET count=count+1 WHERE count<180',u.userId,day());if(!res.meta.changes)throw new Problem(429,'오늘은 충분히 대화했어. 기록을 살펴보고 내일 이어가자.');
...
export async function assignDebate(u:any,context:string,candidates:any[],counts:Record<string,number>){
 const n=Math.min(3,Math.floor(candidates.length/2));if(!n)throw new Problem(400,'준비를 마친 학생이 2명 이상 필요해요.');
 const selected=[...candidates].sort((a,b)=>(counts[a.id]||0)-(counts[b.id]||0)||a.id.localeCompare(b.id)).slice(0,n*2);
